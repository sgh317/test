...
@Controller
@JsBean(value="UserJsBean")
public class UserJsBean {
	...
	public String assignedGroup(String id, String resIds)
	{
		ResultObject ro = new ResultObject();
		...
		if(id != null && !"".equals(id))
		{
			...
			ro = userService.saveUserAssiGroup(id, res);
		}
		else
		{
			ro.setState(false);
			ro.setErrorMessage("用户Id为空");
		}
		
		return JsonUtil.toJson(ro, true);
	}
	
	public String getGroupListById(String userId)
	{
		ResultObject ro = userService.queryGroupListByUserId(userId);
		
		return JsonUtil.toJson(ro, true);
	}
	
	public String getResourceListById(String userId)
	{
		ResultObject ro = userService.queryResourceListByUserId(userId);
