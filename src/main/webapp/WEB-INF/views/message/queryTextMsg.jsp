	...
	RpcContext.getBean('MessageJsBean').queryMessageList(msgKey, msgType, msgFlag, {
    	onSucess:function(rt)
    	{
			...
			if(msgList==null||msgList==""){
				...
				$("#msgListTb").html("<tr><td colspan='11'>暂无文本信息 </td></tr>");
			}else{
				var html = "";
				$.each(msgList,function(index,item){
					html+= "<tr class=\"table-tr\" id='msg_"+item['id']+"'>";
		    		html+= "<td style=\"text-align: center\">"+(index+1)+"</td>";
		    		html+= "<td style=\"text-align: center; width: 100px;\">"+item['title']+"</td>";
		    		html+= "<td style=\"text-align: center; width: 100px;\">"+item['key']+"</td>";
		    		html+= "<td style=\"text-align: center\">"+item['msgContent']+"</td>";
					html+= "<td style=\"text-align: center; width: 100px;\">";
					html+= "<input type='button' class='btn_write' value='编辑' onclick='msgOptionJs.toModifyTextMsg(\""+item['id']+"\")'/>";
...
var msgOptionJs = {
	...
	toGroupInfo:function(id){
		location.href = "${_ContextPath}/group/toShowGroupInfo?id="+id;
	},
	toAssignedGroup:function(id){
		openAssignedGroupDiv();
	} */
};
...
<body>
	<div class="area">&nbsp;&nbsp;当前位置：微信平台 / 消息回复 / <span class="text-red">文本消息</span></div>
    <div>
        <!--左边 开始--->
        <div class="l-content" style="width: 95%;">       
	        <div class="bd_title" style="padding-bottom: 5px;">
	            <div class="tab_menu" style="padding-left: 5px;">
	            	...
	            	<input type="button" id="add" value="新增" class="btn_red" onclick="msgOptionJs.toAddTextMsg();">
				...
				<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#D1D1D1" style="font-size:14px">
					<thead>
						<tr class="table-title">
							...
							<td style="text-align: center; width: 100px;">消息标题</td>
							<td style="text-align: center; width: 100px;">关键字</td>
