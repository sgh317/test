...
function getMessageInfo(id){
	RpcContext.getBean('MessageJsBean').getMessageInfoById(id, {
...
function saveGroup()
{
	...
	RpcContext.getBean('MessageJsBean').saveTextMessage('${id}' ,title, key, content, {
    	onSucess:function(rt)
    	{
    		var res = eval('('+rt+')'); 
...
<body>
<div class="area">&nbsp;&nbsp;当前位置：微信平台 / 消息回复 / <span class="text-red">编辑文本消息</span></div>
    <div>
        <!--左边 开始--->
        <div class="l-content" style="width: 95%;">
            <table cellpadding="0" cellspacing="15" class="table-form">
				...
				<tr>
                    <td></td>
                    <td><input type="button" value="提 交" class="subbut" onclick="saveGroup()">&nbsp;&nbsp;&nbsp;<input type="button" value="取消" class="subbut" onclick="javascript:history.go(-1);"></td>
                </tr>
            </tbody></table>
         </div>
      </div>
