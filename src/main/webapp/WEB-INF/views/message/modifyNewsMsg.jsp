	...
	RpcContext.getBean('MessageJsBean').getMessageInfoById(id, {
    	onSucess:function(rt)
    	{
    		...
    		if(res.state)
    		{
    			...
    			$.each(csa.messageMap.items,function(index,item){
    				item.id = newsIndex;
    				addNewsTr(item.id, item.title, item.picurl, item.url, item.description);
    				newsItem.push(item);
    			});
    		}
    		else
...
function saveNewsMsg()
{
	...
	RpcContext.getBean('MessageJsBean').saveNewsMessage('${id}', title, key, content, {
    	onSucess:function(rt)
    	{
    		var res = eval('('+rt+')'); 
...
<body>
<div class="area">&nbsp;&nbsp;当前位置：微信平台 / 消息回复 / <span class="text-red">编辑图文消息</span></div>
    <div>
        <!--左边 开始--->
